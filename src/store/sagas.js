...
function* api_v1_abcd_listWorker(action) {
  try {
    const result = yield call(apiService.api_v1_abcd_list, action)
    yield put(actions.api_v1_abcd_listSucceeded(result, action))
  } catch (err) {
    yield put(actions.api_v1_abcd_listFailed(err, action))
  }
}
function* api_v1_abcd_listWatcher() {
  yield takeEvery(types.API_V1_ABCD_LIST, api_v1_abcd_listWorker)
}
function* api_v1_abcd_createWorker(action) {
  try {
    const result = yield call(apiService.api_v1_abcd_create, action)
    yield put(actions.api_v1_abcd_createSucceeded(result, action))
  } catch (err) {
    yield put(actions.api_v1_abcd_createFailed(err, action))
  }
}
function* api_v1_abcd_createWatcher() {
  yield takeEvery(types.API_V1_ABCD_CREATE, api_v1_abcd_createWorker)
}
function* api_v1_abcd_readWorker(action) {
  try {
    const result = yield call(apiService.api_v1_abcd_read, action)
    yield put(actions.api_v1_abcd_readSucceeded(result, action))
  } catch (err) {
    yield put(actions.api_v1_abcd_readFailed(err, action))
  }
}
function* api_v1_abcd_readWatcher() {
  yield takeEvery(types.API_V1_ABCD_READ, api_v1_abcd_readWorker)
}
function* api_v1_abcd_updateWorker(action) {
  try {
    const result = yield call(apiService.api_v1_abcd_update, action)
    yield put(actions.api_v1_abcd_updateSucceeded(result, action))
  } catch (err) {
    yield put(actions.api_v1_abcd_updateFailed(err, action))
  }
}
function* api_v1_abcd_updateWatcher() {
  yield takeEvery(types.API_V1_ABCD_UPDATE, api_v1_abcd_updateWorker)
}
function* api_v1_abcd_partial_updateWorker(action) {
  try {
    const result = yield call(apiService.api_v1_abcd_partial_update, action)
    yield put(actions.api_v1_abcd_partial_updateSucceeded(result, action))
  } catch (err) {
    yield put(actions.api_v1_abcd_partial_updateFailed(err, action))
  }
}
function* api_v1_abcd_partial_updateWatcher() {
  yield takeEvery(
    types.API_V1_ABCD_PARTIAL_UPDATE,
    api_v1_abcd_partial_updateWorker
  )
}
function* api_v1_abcd_deleteWorker(action) {
  try {
    const result = yield call(apiService.api_v1_abcd_delete, action)
    yield put(actions.api_v1_abcd_deleteSucceeded(result, action))
  } catch (err) {
    yield put(actions.api_v1_abcd_deleteFailed(err, action))
  }
}
function* api_v1_abcd_deleteWatcher() {
  yield takeEvery(types.API_V1_ABCD_DELETE, api_v1_abcd_deleteWorker)
}
...
export default function* rootSaga() {
  const sagas = [
    api_v1_abcd_listWatcher,
    api_v1_abcd_createWatcher,
    api_v1_abcd_readWatcher,
    api_v1_abcd_updateWatcher,
    api_v1_abcd_partial_updateWatcher,
    api_v1_abcd_deleteWatcher,
