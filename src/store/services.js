...
const mobiledecAPI = axios.create({
  baseURL: "https://mobile-8-dec-dev-16453-prod.herokuapp.com/",
  headers: { Accept: "application/json", "Content-Type": "application/json" }
})
function api_v1_abcd_list(action) {
  return mobiledecAPI.get(`/api/v1/abcd/`)
}
function api_v1_abcd_create(action) {
  return mobiledecAPI.post(`/api/v1/abcd/`, null, { data: action.data })
}
function api_v1_abcd_read(action) {
  return mobiledecAPI.get(`/api/v1/abcd/${action.id}/`)
}
function api_v1_abcd_update(action) {
  return mobiledecAPI.put(`/api/v1/abcd/${action.id}/`, null, {
    data: action.data
  })
}
function api_v1_abcd_partial_update(action) {
  return mobiledecAPI.patch(`/api/v1/abcd/${action.id}/`, null, {
    data: action.data
  })
}
function api_v1_abcd_delete(action) {
  return mobiledecAPI.delete(`/api/v1/abcd/${action.id}/`)
}
